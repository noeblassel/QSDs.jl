...
\maketitle
   ...
   \end{equation}
    ...
    \begin{hypothesis}
        ...
        \end{itemize}
    \end{hypothesis}
    This assumption comes at no cost of generality if we assume~\eqref{eq:supercritical_scaling_delta}. Indeed, we are always free to consider instead~$\deltaRadius{i}(\beta) \land \beta^{\scalingExp-\frac12}$ in the case where~$z_i$ is close to the boundary, and~$\deltaRadius{i} = (1-\shift)\epsBoundary{i}(\beta)\land\beta^{\scalingExp-\frac12}$ for some~$0<\shift<1$ in the case where~$z_i$ is far from the boundary.
    We note that both these choices still satisfy the first condition of~\eqref{eq:supercritical_scaling_delta}, and thus also the third for~$\beta$ large enough.
   ...
   ~$\widetilde\psi_{\beta,k,\alpha}^{(i)}\in H_0^1\cap H^2(\Omega_\beta)$ provided the harmonic eigenmode~$\psi_{\beta,k,\alpha}^{(i)}$ vanishes on~$C_i(\beta)$, where we recall the definition~\eqref{eq:def_ball_cap} of the spherical cap associated with~$z_i$.
    ...
    \begin{proof}[Proof of Theorem~\ref{thm:harm_approx}]
        ...
        \end{equation}
        recalling the definition~\eqref{eq:cutoff}.
        Let us define the superlevel set:
        \[\Omega_\beta^\shift = \{x\in\R^d:\,f_\beta^{\shift}(x)>0\}.\]
        By Assumption~\eqref{eq:supercritical_scaling_delta}, there exists~$\beta_0>0$ such that for all~$\beta>\beta_0$,
        \[B(z_i,\deltaRadius{i})\cap \Omega_\beta \subseteq\localNeighborhood[+]{i} \subseteq B\left(z_i,\delta^{(i)}(\beta)\right) \cap \left[z_i + \halfSpace{i}\left(\frac{\epsLimit{i}+\shift}{\sqrt\beta}\right)\right].\]
