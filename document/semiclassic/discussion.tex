...
\maketitle
   ...
   ~$\widetilde\psi_{\beta,k,\alpha}^{(i)}\in H_0^1\cap H^2(\Omega_\beta)$ provided the harmonic eigenmode~$\psi_{\beta,k,\alpha}^{(i)}$ vanishes on~$C_i(\beta)$, where we recall the definition~\eqref{eq:def_ball_cap} of the spherical cap associated with~$z_i$.
    ...
    \begin{proof}[Proof of Theorem~\ref{thm:harm_approx}]
        ...
        \todo[inline]{Pour pouvoir faire Courant-Fischer ad infinitum, on veut compacité de la resolvante, donc~$\Omega_\beta^\shift$ de bord Lipschitzien}
        \todo[inline]{Dessin!}
    ...
    \section{An Eyring--Krammers formula for~$\lambda_{1,\beta}$.}
    \label{sec:ek}
    We dedicate the study to the derivation of finer asymptotics in the regime~$\beta\to\infty$ for the first Dirichlet eigenvalue of~$-\cL_\beta$,~$\lambda_{\beta,1}$. More precisely, we aim to show the following Eyring--Kramers type formula for~$\lambda_{1,\beta}$:
    \begin{theorem}
        \label{thm:eyring_krammers}
        Assume [...].

        Then the following asymptotic equivalent holds:
        \begin{equation}
            \label{eq:eyring_krammers}
    ...
    \end{theorem}
    As in the proof of the harmonic approximation, the analysis relies on the construction of an accurate approximation of the first Dirichlet eigenvector for~$H_\beta$. 
    The definition of this quasimode, which is closely related to the one performed in~\cite{LPN21}, relies on a local construction around the order one saddle points of~$V$, which is then extended to an element of~$H_0^1(\Omega_\beta)$.
    \subsection{Geometric assumptions}
        ...
        \subsection{Local construction around order-one saddle points}
            ...
            \todo[inline]{Motivation: formellement, avec un potentiel~$\frac12(x_2^2-x_1^2)$, $\xi \equiv 1$, $-\cL_\beta \varphi_\beta = 0$}
